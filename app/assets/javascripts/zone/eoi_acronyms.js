document.addEventListener('DOMContentLoaded', () => {
    ...
    if (document.getElementById("ccs_eoi_acronyms_form") !== null) {
      ...
      for (var acronym_fieldset = 5; acronym_fieldset > 1; acronym_fieldset--) {
        ...
        let this_fieldset = document.querySelector(".acronym_" + acronym_fieldset),
          term_box = document.getElementById("eoi_key_" + acronym_fieldset);
      ...
      deleteButtons.forEach((db) => {
        db.addEventListener('click', (e) => {
          ...
          document.getElementById('eoi_key_' + target).value = "";
          document.getElementById('eoi_key_definition_' + target).value = "";
  ...
  const ccsZvalidateEoiAcronyms = (event) => {
    ...
    for (var x = 1; x < 6; x++) {
      let term_field = document.getElementById('eoi_key_' + x);
      let definition_field = document.getElementById("eoi_key_definition_" + x);
    ...
    if (errorStore.length === 0) document.forms["ccs_eoi_acronyms_form"].submit();
