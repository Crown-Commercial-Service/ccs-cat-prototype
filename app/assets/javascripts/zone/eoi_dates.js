document.addEventListener('DOMContentLoaded', () => {
  ...
  if (document.getElementById('ccs_eoi_response_date_form') !== null) {

    // events and validations related to clarification date
    let clarification_expanded_div = document.getElementById('eoi_clarification_date_expanded');
    let clarification_div = document.getElementById('eoi_clarification_date');
    ...
    clarification_day.addEventListener('blur', (event) => {
      validateEoIDateDay(clarification_day, "clarification_date");
    });
    clarification_month.addEventListener('blur', (event) => {
      validateEoIDateMonth(clarification_month, "clarification_date");
    });
    clarification_year.addEventListener('blur', (event) => {
      validateEoIDateYear(clarification_year, "clarification_date")
    });
    ...
    document.getElementById('save_clarification_date').addEventListener('click', (event) => {
      ...
      if (fieldCheck !== true) {
        ...
        // clarification_expanded_div.classList.add('ccs-dynaform-hidden');
        $.ajax({
          url: "/eoi/changeClarificationDate?date=" + new_date,
          type: "POST",
          ...
          if (result.change_all_dates === true) {
            ...
            document.getElementById('lbl-res-date').innerText = result.eoi_res;
            ...
            document.getElementById('lbl-res-date_copy').innerText = result.eoi_res;
    ...
    let clarification_response_expanded_div = document.getElementById('eoi_clarification_response_date_expanded');
    let clarification_response_div = document.getElementById('eoi_clarification_response_date');
    ...
    clarification_response_day.addEventListener('blur', (event) => {
      validateEoIDateDay(clarification_response_day, "clarification_response_date");
    });
    clarification_response_month.addEventListener('blur', (event) => {
      validateEoIDateMonth(clarification_response_month, "clarification_response_date");
    });
    clarification_response_year.addEventListener('blur', (event) => {
      validateEoIDateYear(clarification_response_year, "clarification_response_date")
    });
    ...
    document.getElementById('save_clarification_response_date').addEventListener('click', (event) => {
      ...
      // clarification_expanded_div.classList.add('ccs-dynaform-hidden');
      $.ajax({
        url: "/eoi/clarification-response?date=" + clarification_res_date,
        type: "POST",
        contentType: "application/json"
      }).done(function (result) {
    ...
    // 4  events and validations related to eoi response date

    let eoi_response_expanded_div = document.getElementById('eoi_response_date_expanded');
    let eoi_response_div = document.getElementById('eoi_response_date');
    eoi_response_expanded_div.classList.add('ccs-dynaform-hidden');
    ...
    document.getElementById('change_eoi_response_date').addEventListener('click', (event) => {
      event.preventDefault();
      eoi_response_expanded_div.classList.remove('ccs-dynaform-hidden');
      eoi_response_div.classList.add('ccs-dynaform-hidden');
    });

    document.getElementById('cancel_eoi_response_date').addEventListener('click', (event) => {
      event.preventDefault();
      eoi_response_expanded_div.classList.add('ccs-dynaform-hidden');
      eoi_response_div.classList.remove('ccs-dynaform-hidden');
    ...
    response_day.addEventListener('blur', (event) => {
      validateEoIDateDay(response_day, "response_date");
    });
    response_month.addEventListener('blur', (event) => {
      validateEoIDateMonth(response_month, "response_date");
    });
    response_year.addEventListener('blur', (event) => {
      validateEoIDateYear(response_year, "response_date")
    });
    ...
    document.getElementById('save_response_date').addEventListener('click', (event) => {
      ...
      $.ajax({
        url: "/eoi/change-response-date?date=" + res_date,
        type: "POST",
        contentType: "application/json"
      }).done(function (result) {
        if (result.error === true) {
    ...
    let ns_expanded_div = document.getElementById('eoi_next_steps_expanded');
    let ns_div = document.getElementById('eoi_next_steps_date');
    ...
    ns_day.addEventListener('blur', (event) => {
      validateEoIDateDay(ns_day, "next_steps_date");
    });
    ns_month.addEventListener('blur', (event) => {
      validateEoIDateMonth(ns_month, "next_steps_date");
    });
    ns_year.addEventListener('blur', (event) => {
      validateEoIDateYear(ns_year, "next_steps_date")
    });
    ...
    document.getElementById('save_next_step_date').addEventListener('click', (event) => {
      ...
      $.ajax({
        url: "/eoi/change-nextsteps-date?date=" + ns_date,
        type: "POST",
        contentType: "application/json"
      }).done(function (result) {
        if (result.error === true) {
