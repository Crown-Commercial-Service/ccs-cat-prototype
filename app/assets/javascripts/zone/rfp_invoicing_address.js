...
const ccsZFindAddress = (event) => {
  ...
  if (fieldCheck !== true) {
    ...
    ccsZPresentErrorSummary([["rfp_invoice_address-postcode", "Enter a valid postcode"]]);
  } else {
    // if we have a valid postcode, remove any error mssages
    ccsZremoveErrorMessage(document.getElementById("rfp_invoice_address-postcode"));
    ...
    let myPostCode = document.getElementById("rfp_invoice_address-postcode").value.trim();
    ...
    fetch(`https://api.getAddress.io/find/${myPostCode}?api-key=''`)
      ...
      .then(data => {
        ...
        let mySelector = document.getElementById("rfp_invoice_address-address"),
        ...
        document.getElementById("rfp_invoice_address-postcode").classList.add("ccs-dynaform-hidden");
...
const ccsZResetAddress = (event) => {
  ...
  document.getElementById("rfp_invoice_address-postcode").classList.remove("ccs-dynaform-hidden");
  document.getElementById("rfi_find_address_btn").classList.remove("ccs-dynaform-hidden");
  document.getElementById("rfp_invoice_address-address").parentNode.classList.add("ccs-dynaform-hidden");
  document.getElementById("rfp_invoice_address-postcode").value = "";
  document.getElementById("rfp_invoice_address-postcode").focus();
