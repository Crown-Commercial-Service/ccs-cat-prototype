const ccsZvalidateRfiDocs = (event) => {
    ...
    if (document.getElementById("rfi_file_upload_1").value.length > 0) {
      fieldCheck = ccsZvalidateWithRegex( "rfi_file_upload_1", "Document type must be one of mentioned file formats",/^(\w|\W)+\.(docx?|pdf|csv|jpg|kml|ods|odt|png|ppt|pptx|rdf|rtf|txt|xls|xlsx|xml|zip)$/);
