document.addEventListener('DOMContentLoaded', () => {
  ...
  if (document.getElementById("ccs_add_rfi_collab") !== null) {
    ...
    deleteButtons.forEach((db) => {
      db.addEventListener('click', function (event) {
        ...
        $.ajax({
          ...
          const node_to_remove = document.getElementById("procurement_collaborator_" + result.id);
          ...
          document.getElementById('procurement_collaborators').appendChild(userOp);
        });
      });
    });

    document.getElementById('procurement_collaborators').addEventListener('change', function (event) {
      event.preventDefault();
    ...
    document.getElementById('ccs_collab_add').addEventListener('click', function (event) {
      ...
      $.ajax({
        url: "/rfi/add-rfi-collaborator?id=" +  document.getElementById('procurement_collaborators').value,
        type: "POST",
        contentType: "application/json",
      //data: JSON.stringify({ id: 2 })
      }).done(function (result) {
      ...
      clone.setAttribute('id', "procurement_collaborator_" + document.getElementById('procurement_collaborators').value);
      let anchorTag = document.createElement('a');
      anchorTag.href = document.getElementById('procurement_collaborators').value;
      ...
      [...document.getElementById('procurement_collaborators').options]
    ...
    const RemoveUser = (event) => {
      ...
      $.ajax({
        ...
        const parent_node = document.getElementById('collaborators-section');
        const node_to_remove = document.getElementById("procurement_collaborator_" + prev_coll);
        parent_node.removeChild(node_to_remove);
        var userOp = document.createElement('option');
        userOp.value = result.id;
        userOp.text = result.fullname;
